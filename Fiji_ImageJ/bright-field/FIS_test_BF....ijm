...
default_radius_filter = 150;
default_subtract_offset = 0.05;
default_threshold = 0.2;
default_enhanceimage = true;
default_fillholes = true;
default_clean = true;
default_label_fontsize = 8;
default_area_min = 2000;
...
testImageNames = newArray("raw", "16bit", "background", "bgCorr_withOffset", "bgCorr_withOffset_rescaled", "bgCorrected", "enhanced", "thresholded", "withoutSaltPepper", "holesFilled", "organoids_beforeQC", "ORGANOIDS_FINAL")
...
while(true) {
	...
	Dialog.addCheckbox("Sharpen image?", default_enhanceimage);
	...
	enhanceimage = Dialog.getCheckbox();
	...
	default_enhanceimage = enhanceimage;
	...
	print("Sharpen image: " + enhanceimage_YN);
	...
	// Image segmentation beginning ======================
		...
		if(bg_filter != "No Filter (flat background)"){
			...
			imageCalculator("Difference create", "16bit","background");
		...
		if(fillholes){
			run("Duplicate...", "title=holesFilled");
			run("Fill Holes");
		}
