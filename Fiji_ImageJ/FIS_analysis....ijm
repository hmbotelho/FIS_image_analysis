...
#@ Float (label="Manual threshold value", value=0.050) threshold
...
for(dirnum=0; dirnum<allfolders.length; dirnum++){
	...
	for(imgnum=0; imgnum<ok_images.length; imgnum++){
		...
		// Segment image
		segmentObjects(bg_filter, radius_filter, subtract_offset, threshold, clean, true);
...
function segmentObjects(bg_filter, radius_filter, subtract_offset, threshold, clean, keepBgCorr) {
